...
class _StudentRegistrationScreenState extends State<StudentRegistrationScreen> {
  ...
  Future<void> _handleRegister() async {
    ...
    if (_selectedDepartment == null || _selectedYear == null || _selectedSection == null) {
      setState(() => _errorMessage = 'Please select Department, Academic Year, and Section.');
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Student registered successfully! Please sign in.'),
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: 'e.g. 717822P101',
                    prefixIcon: Icon(Icons.pin_outlined),
                  ),
                  validator: (val) => val == null || val.trim().isEmpty ? 'Register Number is required' : null,
                ),
                const SizedBox(height: 16),
                ...
                // Year Selection (Dependent on Department)
                if (_selectedDepartment != null) ...[
                  StreamBuilder<List<AcademicYearModel>>(
                    stream: _firestoreService.getYears(_selectedDepartment!.id),
                    builder: (context, snapshot) {
                      ...
                      return DropdownButtonFormField<AcademicYearModel>(
                        ...
                        items: years.map((y) {
                          return DropdownMenuItem(value: y, child: Text(y.name));
                        }).toList(),
                        onChanged: (val) {
                          setState(() {
                            _selectedYear = val;
                ...
                // Section Selection (Dependent on Department + Year)
                if (_selectedDepartment != null && _selectedYear != null) ...[
                  StreamBuilder<List<SectionModel>>(
                    stream: _firestoreService.getSections(_selectedDepartment!.id, _selectedYear!.id),
                    builder: (context, snapshot) {
                      ...
                      return DropdownButtonFormField<SectionModel>(
                        ...
                        items: sections.map((sec) {
                          return DropdownMenuItem(value: sec, child: Text(sec.name));
                        }).toList(),
