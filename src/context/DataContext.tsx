  ...
  useEffect(() => {
    const unsubs = [
      ...
      onSnapshot(collection(db, 'attendance_sessions'), s => {
        setAttendanceSessions(s.docs.map(d => ({ id: d.id, ...d.data() } as AttendanceSession)));
      ...
      onSnapshot(collection(db, 'assignment_submissions'), s => {
        setAssignmentSubmissions(s.docs.map(d => ({ id: d.id, ...d.data() } as AssignmentSubmission)));
      }, err => console.warn('Firestore sub err:', err)),
      onSnapshot(collection(db, 'notes'), s => {
        setNotes(s.docs.map(d => ({ id: d.id, ...d.data() } as NoteItem)));
      }, err => console.warn('Firestore note err:', err)),
      onSnapshot(collection(db, 'study_sessions'), s => {
        setStudySessions(s.docs.map(d => ({ id: d.id, ...d.data() } as StudySession)));
      ...
      onSnapshot(collection(db, 'notifications'), s => {
        setNotifications(s.docs.map(d => ({ id: d.id, ...d.data() } as AppNotification)));
      }, err => console.warn('Firestore notif err:', err)),
      onSnapshot(collection(db, 'planner_tasks'), s => {
        setPlannerTasks(s.docs.map(d => ({ id: d.id, ...d.data() } as StudyPlannerTask)));
